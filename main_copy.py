...
#facciones_cp= facciones_cp.astype({'Valores_poder':'object'})
...
def fig_neutral():
    ...
    #p12 = figure(x_range=(0,1), y_range=(0,1),plot_width=250, plot_height=300)
    ...
    #p12.image_url(url=['/static/Neutral.png'], x=0, y=1, w=1, h=1)
    #p12.image_url(url=['C:/Users/alexs/Desktop/Alejandro/Universidad/Master_Ciencia_de_datos/Gwent-TFM/bokeh_gwent/static/Neutral.png'], x=0, y=1, w=1, h=1)
    #p12.image_url(url=['https://static.bokeh.org/logos/logo.png'], x=0, y=1, w=1, h=1)
...
def figura2():
    #dataframe_faction_cartas = counts(seleccion(df_gwent,'FactionId','Neutral'),'Type')
    ...
    #p3.xaxis.axis_label_text_font_size = '30pt'
    #p41.xaxis.axis_label_text_font_size = '15pt'
    #p41.yaxis.axis_label_text_font_size = '15pt'
    ...
    #p3.xaxis.axis_label_text_font_size = '30pt'
    #p42.xaxis.axis_label_text_font_size = '15pt'
    #p42.yaxis.axis_label_text_font_size = '15pt'
    ...
    #p3.xaxis.axis_label_text_font_size = '30pt'
    #p43.xaxis.axis_label_text_font_size = '15pt'
    #p43.yaxis.axis_label_text_font_size = '15pt'
    ...
    #p3.xaxis.axis_label_text_font_size = '30pt'
    #p44.xaxis.axis_label_text_font_size = '15pt'
    #p44.yaxis.axis_label_text_font_size = '15pt'
...
#source = ColumnDataSource(data=data_table)
...
tab6 = Panel(child=row(column(data_table1,button1),column(data_table2,button2)),title='Dataframe')
    ...
    def counts(self,atributo):
        ...
        if atributo =='Abilites':
            ...
            for value in lista_habilidades:
                ...
                cuentas = pd.DataFrame(data.groupby(value).size()).reset_index().rename(columns = {value:'Hab',0:'Cuenta'}).fillna(0)
                dataframe_cuentas.append(cuentas)

            union = dataframe_cuentas[0].merge(dataframe_cuentas[1], how = 'outer',on = 'Hab')
            union = union.merge(dataframe_cuentas[2], how = 'outer',on = 'Hab')
            union = union.merge(dataframe_cuentas[3], how = 'outer',on = 'Hab')
            union = union.merge(dataframe_cuentas[4], how = 'outer',on = 'Hab')
            union = union.merge(dataframe_cuentas[5], how = 'outer',on = 'Hab')
            union = union.merge(dataframe_cuentas[6], how = 'outer',on = 'Hab')
            union = union.merge(dataframe_cuentas[7], how = 'outer',on = 'Hab')
            union =union.fillna(0)
            union['Cuenta'] = union.sum(axis =1)
            union = union[['Hab','Cuenta']].sort_values(by = ['Cuenta'],ascending = False)
            return union
            ...
            for value in lista_habilidades:
                ...
                cuentas = pd.DataFrame(data.groupby(value).size()).reset_index().rename(columns = {value:'Cat',0:'Cuenta'}).fillna(0)
                dataframe_cuentas.append(cuentas)

            union = dataframe_cuentas[0].merge(dataframe_cuentas[1], how = 'outer',on = 'Cat')
            union = union.merge(dataframe_cuentas[2], how = 'outer',on = 'Cat')
            
            union =union.fillna(0)
            union['Cuenta'] = union.sum(axis =1)
            union = union[['Cat','Cuenta']].sort_values(by = ['Cuenta'],ascending = False)
            return union


    data_x = counts(seleccion(seleccion(df_gwent,'FactionId',y_x.value),'Type',y2_x.value),'Categories')[1:]
    dataframe1 = data_x
    cate =list(dataframe1['Cat'])
...
def figura2_y():
    ...
    data_y = counts(seleccion(seleccion(df_gwent,'FactionId',y_x.value),'Type',y2_x.value),'Abilites')
    dataframe1 = data_y
    Abi =list(dataframe1['Hab'])
    counts =list(dataframe1['Cuenta'])
...
def update_habilidad(attr, old, new):
    ...
    #texto = keywords[keywords['Ability']==new]['Description'][0]
    #habilidad=Div(text=texto)
    #habilidad.text = new
def texto():
    os.chdir(r'c:\\Users\\alexs\\Desktop\\Alejandro\\Universidad\\Master_Ciencia_de_datos\\Gwent-TFM\\bokeh_gwent\\data')
    keywords = pd.read_excel('keywords.xlsx')
